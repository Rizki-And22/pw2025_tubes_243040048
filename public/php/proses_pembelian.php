...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nama) || empty($kontak) || empty($email) || empty($metode) || empty($kategori) || empty($jumlah) || empty($harga) || empty($subtotal)) {
        ...
        if ($stok_akhir <= 0) {
            ...
            $insert_query = "INSERT INTO penjualan_tiket (metode_pembayaran, jumlah_tiket, nama_pemesan, kontak_pemesan,  email, ID_Tiket, harga, total_harga, kode_booking)
                         VALUES ('$metode', '$jumlah', '$nama', '$kontak', '$email','$kategori', '$harga', '$subtotal', 'BK-" . time() . "')";
